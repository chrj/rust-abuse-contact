...
/// sent there is noise.
///
/// An IPv4-mapped address, such as `::ffff:10.0.0.1`, is judged as the IPv4 address it
/// carries. Without that, the IPv6 spelling of a private address would pass.
///
/// ```
...
/// before this table is read, so the IPv4 table judges it. The last row is not in that
/// registry: it is the multicast range, from the multicast address registry.
